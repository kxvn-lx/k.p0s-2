...
const getInputClassNames = (editable: boolean | undefined, className?: string) => {
  return cn(
    "font-mono dark:bg-input/30 border-input bg-background text-foreground h-10 flex w-full min-w-0 flex-row items-center rounded-[--radius] border px-2 shadow-sm shadow-black/5 sm:h-9 text-base leading-none",
    editable === false &&
    cn(
      "opacity-50",
      Platform.select({
