...
interface PenjualanDetailContentProps {
    staffName: string
    tanggal: string
    jumlahTotal: number
...
export function PenjualanDetailContent({
    staffName,
    tanggal,
    jumlahTotal,
    keterangan,
    details,
}: PenjualanDetailContentProps) {
    return (
                ...
                <View className="border border-border bg-background">
                    ...
                    <View className="p-2 gap-2">
                        ...
                        <View className="flex-row justify-between items-center">
                            ...
                            <Text className="text-primary font-mono-bold text-lg">
                            ...
                            <View key={item.id}>
                                ...
                                <View className="p-2 gap-2">
                                    <View className="flex-row justify-between items-start">
                                        ...
                                        <Text className="text-primary font-mono-bold text-sm">
                                            {item.jumlah_total.toLocaleString("id-ID")}
                                        </Text>
                                    </View>
                                    <View className="flex-row justify-between">
                                        <Text className="text-muted-foreground font-mono text-xs uppercase">
                                            {item.qty} {item.satuan_utama || "PCS"} @ {item.harga_jual.toLocaleString("id-ID")}
                                        </Text>
                                    </View>
                                    {item.variasi && (
